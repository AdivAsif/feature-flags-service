...
const API_KEY_COUNT = Number(__ENV.API_KEY_COUNT || 100);

// Hot/cold split for API keys (separate from flags/users)
const HOT_API_KEY_PERCENTAGE = Number(__ENV.HOT_API_KEY_PERCENTAGE || 0.2);
...
export function setup() {
    ...
    for (let projectIndex = 0; projectIndex < PROJECT_COUNT; projectIndex++) {
        const projectName =
            PROJECT_COUNT === 1
                ? PROJECT_NAME_PREFIX
                : `${PROJECT_NAME_PREFIX}_${String.fromCharCode(97 + projectIndex)}`;

        const project = getOrCreateProject(api, adminToken, projectName);

        // CREATE MULTIPLE API KEYS PER PROJECT
        for (let i = 0; i < API_KEY_COUNT; i++) {
            const keyName = `${projectName}_key_${i}`;
            const apiKey = getOrCreateApiKey(api, adminToken, project.id, keyName);
            createdApiKeys.push(apiKey);
        }

        const flagKeys = getOrCreateFlags(api, createdApiKeys[0], project.id, FLAGS_PER_PROJECT);
    ...
    return {
        ...
        hotApiKeyCount: Math.max(1, Math.ceil(createdApiKeys.length * HOT_API_KEY_PERCENTAGE)),
...
export function evaluate(data) {
    ...
    const isHotKey = (next() / 0xffffffff) < HOT_TRAFFIC_PERCENTAGE;

    const apiKey = isHotKey
        ? data.apiKeys[next() % data.hotApiKeyCount]
        : data.apiKeys[next() % data.apiKeys.length];
